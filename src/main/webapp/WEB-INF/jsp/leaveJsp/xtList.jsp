...
<body style="width:100%;">
  <div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20"> 
	  <form id="queryForm" action="<%=basePath%>/leave/xtList.do" method="post" class="form-inline">
		<div class="text-c"> 
		    ...
		    <div id="treediv" style="text-align:left; display: none;position:relative; z-index:99; overflow:scroll;  width:200px;height:300px;  padding: 5px;background: #fff;color: #fff;border: 1px solid #cccccc;">
		     ...
		     审批状态：<select name="spState" id="spState" class="input-text radius" style="width:150px;">
		           <option></option>
	...
	<c:when test="${! empty pageResult.list}">
	   <table class="table table-border table-bordered table-bg table-hover">
		<thead>
			<tr>
				<th scope="col" colspan="11"><center><font size='2'>系统请假单列表</font></center></th>
			</tr>
			<tr class="text-c">
			    <th width="5%">序号</th>
			    <th>请假人</th>
            ...
            <td colspan="13">
              ...
              <c:choose>
                <c:when test="${pageResult.page.hasPrePage}">
                  <a href="<%=basePath%>leave/xtList.do?currentPage=1&ZSXM=${ZSXM}&did=${did}&pId2=${pId2}&spSate=${spSate}"><font size='2'>首页</font></a> | 
                  <a href="<%=basePath%>leave/xtList.do?currentPage=${pageResult.page.currentPage-1}&ZSXM=${ZSXM}&did=${did}&pId2=${pId2}&spSate=${spSate}"><font size='2'>上一页</font></a>
                </c:when>
                <c:otherwise>
              ...
              <%
				for(int i=begin;i<=end;i++){
			     ...
			     %>
			           <a href='<%=basePath%>leave/xtList.do?currentPage=<%=i%>&ZSXM=${ZSXM}&did=${did}&bmmc=${bmmc}&spSate=${spSate}'><font size='2'>[<%=i%>]</font></a>   
			     <%	  
				     }
				}
			   %> 
               <c:choose>
                <c:when test="${pageResult.page.hasNextPage}">
                  <a href="<%=basePath%>leave/xtList.do?currentPage=${pageResult.page.currentPage+1}&ZSXM=${ZSXM}&did=${did}&pId2=${pId2}&spSate=${spSate}"><font size='2'>下一页</font></a> | 
                  <a href="<%=basePath%>leave/xtList.do?currentPage=${pageResult.page.totalPage}&ZSXM=${ZSXM}&did=${did}&pId2=${pId2}&spSate=${spSate}"><font size='2'>尾页</font></a>
                </c:when>
                <c:otherwise>
...
function tiaozhuan(){
	  ...
	  window.location.href='<%=basePath%>leave/xtList?&ZSXM=${ZSXM}&did=${did}&pId2=${pId2}&spSate=${spSate}&currentPage='+currentPage+'';
}
</script>
