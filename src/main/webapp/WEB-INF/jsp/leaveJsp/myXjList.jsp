  ...
  <div class="page-container">
	...
	<c:when test="${! empty pageResult.list}">
	   <table class="table table-border table-bordered table-bg table-hover">
		<thead>
			<tr>
				<th scope="col" colspan="9"><center><font size='2'>我的已通过审批的请假单</font></center></th>
			</tr>
			<tr class="text-c">
			    <th width="5%">序号</th>
				<th>请假开始时间</th>
	    ...
	    <tr>
		  ...
		  <td><center>
		    <c:if test="${q.sfxj=='01'}"><a onclick="xjdetail('销假详情','<%=basePath%>/leave/xjdetail.do?id=${q.id}','650','350')" style="color:blue;">已销假</a></c:if>
		  ...
		  <td>
		    <center>
		      <c:if test="${q.sfxj=='00'}">
		         <a onclick="xiaojia('销假','<%=basePath%>/leave/xjUI.do?id=${q.id}','650','350')" style="color:blue;">销假</a>&nbsp;
		      </c:if>
            ...
            <td colspan="13">
              ...
              <c:choose>
                <c:when test="${pageResult.page.hasPrePage}">
                  <a href="<%=basePath%>leave/myXjList.do?currentPage=1"><font size='2'>首页</font></a> | 
                  <a href="<%=basePath%>leave/myXjList.do?currentPage=${pageResult.page.currentPage-1}"><font size='2'>上一页</font></a>
                </c:when>
                <c:otherwise>
              ...
              <%
				for(int i=begin;i<=end;i++){
			     ...
			     %>
			           <a href='<%=basePath%>leave/myXjList.do?currentPage=<%=i%>'><font size='2'>[<%=i%>]</font></a>   
			     <%	  
				     }
				}
			   %> 
               <c:choose>
                <c:when test="${pageResult.page.hasNextPage}">
                  <a href="<%=basePath%>leave/myXjList.do?currentPage=${pageResult.page.currentPage+1}"><font size='2'>下一页</font></a> | 
                  <a href="<%=basePath%>leave/myXjList.do?currentPage=${pageResult.page.totalPage}"><font size='2'>尾页</font></a>
                </c:when>
                <c:otherwise>
...
function tiaozhuan(){
	  ...
	  window.location.href='<%=basePath%>leave/myXjList?currentPage='+currentPage+'';
}
</script>
</body>
</html>