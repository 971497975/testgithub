...
<body style="width:100%;">
  <div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20"> 
	  <form id="queryForm" action="<%=basePath%>/leave/xtxjList.do" method="post" class="form-inline">
		<div class="text-c"> 
	...
	<c:when test="${! empty pageResult.list}">
	   <table class="table table-border table-bordered table-bg table-hover">
		<thead>
			<tr>
				<th scope="col" colspan="11"><center><font size='2'>系统已通过审批的请假单</font></center></th>
			</tr>
			<tr class="text-c">
			    <th width="5%">序号</th>
			    <th>请假人</th>
            ...
            <td colspan="13">
              ...
              <c:choose>
                <c:when test="${pageResult.page.hasPrePage}">
                  <a href="<%=basePath%>leave/xtxjList.do?currentPage=1"><font size='2'>首页</font></a> | 
                  <a href="<%=basePath%>leave/xtxjList.do?currentPage=${pageResult.page.currentPage-1}"><font size='2'>上一页</font></a>
                </c:when>
                <c:otherwise>
              ...
              <%
				for(int i=begin;i<=end;i++){
			     ...
			     %>
			           <a href='<%=basePath%>leave/xtxjList.do?currentPage=<%=i%>'><font size='2'>[<%=i%>]</font></a>   
			     <%	  
				     }
				}
			   %> 
               <c:choose>
                <c:when test="${pageResult.page.hasNextPage}">
                  <a href="<%=basePath%>leave/xtxjList.do?currentPage=${pageResult.page.currentPage+1}"><font size='2'>下一页</font></a> | 
                  <a href="<%=basePath%>leave/xtxjList.do?currentPage=${pageResult.page.totalPage}"><font size='2'>尾页</font></a>
                </c:when>
                <c:otherwise>
...
function tiaozhuan(){
	  ...
	  window.location.href='<%=basePath%>leave/xtxjList?currentPage='+currentPage+'';
}
</script>
